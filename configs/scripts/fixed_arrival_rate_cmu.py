...
def lam_torch(env, t: torch.Tensor) -> torch.Tensor:
    """
    t: [B,1], returns arrival rates of shape [B,Q] (>0), on env.device
    """
    device = env.device
    B = t.shape[0]
    Q = env.Q
...
def draw_service(env, time: torch.Tensor) -> torch.Tensor:
    ...
    If env_config['service_type']=='hyper', uses a mixture of exponentials with two means; otherwise defaults to Exp(mean=1)
    This generates the workload, which is divided by (action times mu service rate) in the env
    """
    device = env.device
...
def draw_inter_arrivals(env, time: torch.Tensor) -> torch.Tensor:
    """
    Returns the next inter-arrival times of shape [B,Q] (how long until the next arrival occurs): Exp(rate=λ) => Exp(1)/λ
    """
    device = env.device
    B, Q = time.shape[0], env.Q
    U = torch.rand((B, Q), device=device).clamp_min(1e-6)
